...
SubgraphNode CompGraph::generateSubgraph(
    ...
    if (level >= set->arity() - 1) {
        ...
        for (auto& pair : stmtGroups) {
            ...
            if (child.val != -1) {
                sgNode.children.push_back(child);
            }
        }
    }
#ifdef DEBUG
...
void CompGraph::reducePCRelations() {
    ...
    */
    ...
    while (sIt != stmtNodes.end()) {
        ...
        if (d->in.size() != 1 || d->out.size() != 1) {
            continue;
        }

        // Make sure s1, d are not in subgraphs
        if (s1->subgraph != 0 || d->subgraph != 0) {
            continue;
        }
...
std::string CompGraph::partialDotString(std::shared_ptr<StmtNode> stmtNode,
    ...
    if (stmtNode) {
        ...
        if (reads) {
            stmtRQueue.push(stmtNode);
        }
        if (writes) {
            stmtWQueue.push(stmtNode);
        }
    } else {
        // Draw source node with special node type
        ...
        if (reads) {
            dataRQueue.push(dataNode);
        }
        if (writes) {
            dataWQueue.push(dataNode);
        }
    }
...
void CompGraph::subgraphDotString(std::ostringstream& os,
    ...
    for (auto it = order.begin(); it != order.end(); it++) {
        ...
        if (it == order.begin() || it == order.end() - 1) {
            continue;
        }
        // Start the subgraph
...
void CompGraph::resolvePhiNodes(std::vector<int>& deletStmtIds,
                     std::vector<std::pair<std::string,std::string>>&
                     dataMacros){
for (auto& pair : dataNodes) {
    if (pair.second->type == NodeTypes::unread
            || pair.second->type == NodeTypes::activeOut) {
        std::queue<std::shared_ptr<DataNode>> dataQueue;
        dataQueue.push(pair.second);
        while (!dataQueue.empty()) {
            auto dataNode = dataQueue.front();
            ...
            // dataNode must be a leaf node
            //if (dataNode->out.size() != 0) {
                // Add this data node to the back of
		// the data queue so we can automatically
		// resolve statements that depends on it
	    ...
	    //	continue;
            ...
            while (dInIt != dataNode->in.end()) {
                auto stmtNode = dInIt->lock();
                // Skip statment nodes that are not
                // define phis and that are not merge
                // phis. They are uninteresting.
                if (!stmtNode->isDefPhi && !stmtNode->isPhiNode){
                    dInIt++;
		    continue;
		}
                // Check if stmtNode is a define phi
                std::string unversionedData = 
			SSA::getUnversionedDataSpace(dataNode->id);
                dataMacros.push_back({dataNode->id,unversionedData});
		...
		// Disconnect stmtNode and dataNode
                ...
                // Store the statement id
                deletStmtIds.push_back(stmtNode->id);
...
#ifdef DEBUG
std::cerr << "Total Data Nodes Alias: " << dataMacros.size()
          << "\n Total Stmt Nodes to be removed: " << deletStmtIds.size()
          << "\n";
#endif
}
}
